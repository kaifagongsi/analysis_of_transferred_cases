 ...
 * 加副分率
 */
function assCodeRate(page,rows) {
    let infoForm = $.param({"rows":rows}) + "&" +  $.param({"page":page}) + "&" + $("#infoForm").serialize();
    if(vaildateForm()){
        loadingSpinner = layer.msg('正在加载...', {icon: 16, shade: 0.3, time:0});
        $.post(ctx + "/count/asscoderate/getAssCodeRate", infoForm,function (response) {
            tableThead = [];
            tableData = [];
            if(response.flag){
                // 获取表头：
                ...
                laypage.render({
                    ...
                    ,jump: function(obj, first){
                        ...
                        //首次不执行
                        if(!first){
                            assCodeRate( obj.curr,obj.limit);
                        }
                    }
                });
            }else{
 ...
 * @returns {boolean}
 */
function caseOutOfDay(page,rows) {
    let infoForm = $.param({"rows":rows}) + "&" +  $.param({"page":page}) + "&" + $("#infoForm").serialize();
    if(vaildateForm()){
        loadingSpinner = layer.msg('正在加载...', {icon: 16, shade: 0.3, time:0});
            ...
            if(response.flag){
                ...
                laypage.render({
                    ...
                    ,jump: function(obj, first){
                        ...
                        //首次不执行
                        if(!first){
                            caseOutOfDay( obj.curr,obj.limit);
                        }
                    }
                });
            }else{
 ...
 * @returns {boolean}
 */
function caseOutOfWeek(page,rows) {
    let infoForm = $.param({"rows":rows}) + "&" +  $.param({"page":page}) + "&" + $("#infoForm").serialize();
    if(vaildateForm() && checkWeek()){
        loadingSpinner = layer.msg('正在加载...', {icon: 16, shade: 0.3, time:0});
            ...
            if(response.flag){
                ...
                laypage.render({
                    ...
                    ,jump: function(obj, first){
                        ...
                        //首次不执行
                        if(!first){
                            caseOutOfWeek( obj.curr,obj.limit);
                        }
                    }
                });
            }else{
 ...
 * @returns {boolean}
 */
function caseOutOfMonth(page,rows) {
    let infoForm = $.param({"rows":rows}) + "&" +  $.param({"page":page}) + "&" + $("#infoForm").serialize();
    if(vaildateForm() && checkMonth()){
        loadingSpinner = layer.msg('正在加载...', {icon: 16, shade: 0.3, time:0});
            ...
            if(response.flag){
                ...
                laypage.render({
                    ...
                    ,jump: function(obj, first){
                        ...
                        //首次不执行
                        if(!first){
                            caseOutOfMonth( obj.curr,obj.limit);
                        }
                    }
                });
            }else{
